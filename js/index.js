...
function loadData(){
  ...
  for (let i = 0; i < json.length; i++) {
    ...
    //create h3 with name from obj
    var nameH3 = document.createElement("h3");
    nameH3.id = "name";
    nameH3.innerHTML = json[i].name;
    //create p with obj description
    var descriptionP = document.createElement("p");
    var type = document.createElement("i");
    var price = document.createElement("i");
    var priceP = document.createElement("p");
    var typeP = document.createElement("p");
    type.classList.add("fas", "fa-home");
    price.classList.add("fas", "fa-dollar-sign");
    priceP.innerHTML = " "+ json[i].price+",00 ";
    typeP.innerHTML = " "+ json[i].property_type;
    var button = document.createElement("button");
    button.innerHTML = "ALUGUE";
    button.classList.add("btn","btn--3");
    button.onclick = function(){
      alert("É apenas uma demonstração, este componente não possui funcionalidade implementada.");
    };
    //"appendation"
    priceP.appendChild(price);
    typeP.appendChild(type);
    descriptionP.appendChild(typeP);
    descriptionP.appendChild(priceP);
    descriptionP.appendChild(button);
    contentDIV.appendChild(nameH3);  
    contentDIV.appendChild(descriptionP);
    boxDIV.appendChild(imgBxDIV);
    boxDIV.appendChild(contentDIV);
    cards.appendChild(boxDIV);
  }
}
